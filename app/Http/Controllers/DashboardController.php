    ...
    public function __invoke()
    {
        ...
        if ($user->isRole('staff_gudang')) {
            return view('dashboard', [
                'role' => 'staff_gudang',
                'incomingToday' => StockIn::with('product')->whereDate('date', $date)->latest()->get(),
                'outgoingToday' => StockOut::with('product')->whereDate('date', $date)->latest()->get(),
            ]);
        }
        ...
        return view('dashboard', [
            ...
            'incomingTodayCount' => StockIn::whereDate('date', $date)->sum('quantity'),
            'outgoingTodayCount' => StockOut::whereDate('date', $date)->sum('quantity'),
            'lowProducts' => Product::with('category')->whereColumn('stock', '<=', 'min_stock')->orderBy('stock')->take(6)->get(),
            'activities' => $user->isRole('admin') ? ActivityLog::with('user')->latest()->take(8)->get() : collect(),
            'chartLabels' => $labels,
